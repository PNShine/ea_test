...
export default class Main extends React.Component {
    ...
    renderDivs() {
        // get keys and values as arrays for easy traversal
        ...
        return <div id="main">{
            keys.map((make, index) => {
                ...
                return (<div className="make drop-shadow" key={index}>{make}{
                    keys2.map((model, index2) => {
                        let shows = Array.from(values2[index2]).toString();
                        return <div className="models" key={index2}>{model + ": " + shows}<br /></div>
                    })
                }<br />
                </div>)
            })
    ...
    render() {
        ...
        if (this.state.isLoaded) {
            if (isNullOrUndefined(this.state.data)) {
                div = (<Error msg={"Failed to fetch data:" + this.state.err} />);
            }
            else if (this.state.data == "") {
                div = (<Error msg="Empty dataset recieved" />);
            }
            else {
                div = this.renderDivs();
            }
        }
        return <div id="container"><div id="title">Energy Australia Test</div>{div}</div>;
    }
}